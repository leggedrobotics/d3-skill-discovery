...
class MeshRandomBoxesTerrainCfg(SubTerrainBaseCfg):
    ...
    """The number of boxes to place on the terrain. Defaults to (10, 10)."""

    box_height_range: tuple[float, float] = (0.1, 0.5)
    """Min and max height of each randomly generated box. Defaults to (0.1, 0.5)."""

    box_width_range: tuple[float, float] = (0.2, 1.0)
    """Min and max width (size in X direction) of each randomly generated box. Defaults to (0.2, 1.0)."""

    box_depth_range: tuple[float, float] = (0.2, 1.0)
    """Min and max depth (size in Y direction) of each randomly generated box. Defaults to (0.2, 1.0)."""

    platform_width: float = 1.0
    """The width of the central square platform area (in meters) where no boxes are spawned. Defaults to 1.0."""
...
class MeshFourWallsTerrainCfg(SubTerrainBaseCfg):
    ...
    """Thickness of the flat floor in meters. Defaults to 0.0."""

    wall_height: float = 1.0
    """Height of each wall in meters. Defaults to 1.0."""

    wall_length: float = 1.0
    """Length of each wall segment in meters. Defaults to 1.0.

    If equal to ``size[0]`` (the terrain size), the terrain is fully enclosed. If less than ``size[0]``, the corners remain open.
    """

    wall_thickness: float = 0.1
    """Thickness (width) of the wall segments in meters. Defaults to 0.1."""
