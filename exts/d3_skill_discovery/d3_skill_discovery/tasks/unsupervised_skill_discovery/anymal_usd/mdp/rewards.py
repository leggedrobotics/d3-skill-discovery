...
from d3_skill_discovery.tasks.unsupervised_skill_discovery.anymal_usd.mdp.nets import NET_PATH
...
def base_below_min_height(
    ...
    mean_heights = asset.data.root_pos_w[:, 2] - hits
    return torch.square(torch.clamp(target_height - mean_heights, min=0, max=1))


class cost_of_transportation(ManagerTermBase):
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.sum_energy_per_dist = torch.zeros(env.num_envs).to(env.device)
    ...
    def __call__(self, env, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
        ...
        joint_torques = asset.data.applied_torque[:, asset_cfg.joint_ids].abs()
        joint_velocities = asset.data.joint_vel[:, asset_cfg.joint_ids].abs()
        power = torch.sum(joint_torques * joint_velocities, dim=1)
        self.sum_energy_per_dist += power * env.step_dt

        to_reset = env.episode_length_buf == 0
        if to_reset.any():
            self.sum_energy_per_dist[to_reset] = 0.0

        # calculate distance
        distance = torch.linalg.vector_norm(asset.data.spawn_pose[:, :2] - asset.data.root_pos_w[:, :2], dim=1)
        return self.sum_energy_per_dist / (distance + 1e-3)


def applied_torque_limits_fraction(
    env: ManagerBasedRLEnv, fraction: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize applied torques if they cross a fraction of the limits.

    This is computed as a sum of the absolute value of the difference between the applied torques and the limits times a fraction.
    ...
    out_of_limits = torch.clamp(
        asset.data.computed_torque[:, asset_cfg.joint_ids]
        - fraction * asset.actuators["legs"].effort_limit[:, asset_cfg.joint_ids],
        min=0,
    )
