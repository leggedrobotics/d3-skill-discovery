...
class SkillSelectionAction(ActionTerm):
    ...
    def __init__(self, cfg: SkillSelectionActionCfg, env: ManagerBasedRLEnv):
        ...
        self._action_dim = sum([v[1] for v in cfg.skill_space.values()])
    ...
    def process_actions(self, actions):
        ...
        for skill_name, (distr_type, dim) in self.cfg.skill_space.items():
            ...
            if distr_type == "categorical":
                ...
                self._processed_command_actions[:, dim_idx:next_dim_idx].copy_(
                ...
                # copy the action
                self._processed_command_actions[:, dim_idx:next_dim_idx].copy_(actions[:, dim_idx:next_dim_idx].float())
            else:
