    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                # future_idx = (batch_idx + self.num_envs) % batch_size
                ...
                if isinstance(self.observations, dict):
                    ...
                    # future_obs_batch = {key: observations[key][future_idx] for key in observations}
                    # same_skills = (future_obs_batch["skill"] == obs_batch["skill"]).all(dim=1)
                    # diff = torch.norm((future_obs_batch["my_pose"] - obs_batch["my_pose"])[:, :2], dim=1)
